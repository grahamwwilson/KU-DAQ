...
//  pdatafile.open("Data.out"); 
  ...
  gettimeofday(&tim, NULL);
  t1 = tim.tv_sec + (tim.tv_usec/1000000.0);
  printf("Initialization finished: %.6lf s\n",t1);
...
*/
  ...
  int nread = 0;
  int ntoread = NWORDS;              // Read the 4 words that need to be read. 
...
*/
  ...
  int k=2;                              // Discard first two words
  while (k < NWORDS){
